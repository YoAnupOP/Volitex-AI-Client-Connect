...
async function main() {
  ...
  if (!email || !/^\S+@\S+\.\S+$/.test(email) || !password || password.length < 12 || !["owner", "operator"].includes(role)) throw new Error("Usage: npm run create-admin -- <email> <12+ character password> [--role owner|operator]");
