...
export async function POST(request: NextRequest) {
  try {
    ...
    const tenantIds = await removeWhatsappConnectionByMetaUserId(payload.user_id, "deauthorized");
    console.info("WhatsApp connection deauthorized", { tenantCount: tenantIds.length });
    ...
    console.error("Unable to process WhatsApp deauthorization", error);
